	...
	fillForm: function(options){
		...
		if ( options.data ) {
			$.each(options.data, function(index, value) {
				...
				isDateField = form_element_prop=='datetime' || form_element_prop=='date' || form_element_prop=='time' || $form_element.hasClass('hasDatepicker');
				isCheckbox = form_element_prop=='checkbox' || form_element_prop=='radio';

				if ( isDateField ) {
					$form_element.val(new Date(value * 1000).toJSON().slice(0,10));
				} else if ( isCheckbox ) {
...
EllyCore.contextMenu({
	...
	contextMenu: function(options){
		...
		/* Всплывающий мультиселект из jQuery UI. Документация и допустимые параметры: http://www.erichynds.com/blog/jquery-ui-multiselect-widget
EllyCore.multiselect({
	element: $('ID_селектбокса'),
});
		*/
	multiselect: function(options){
		var options = $.extend({}, EllyCore.defaults.multiselect, options);
